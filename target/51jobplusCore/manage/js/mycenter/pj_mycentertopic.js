        	    ...
        	    success:function(data){
        	      if(data.returnStatus=='000'){//返回成功
        	    	...
        	    	var topicdata={
       	    		...
       	    		$('.pagetemplate').setTemplateURL(projectName+'manage/template/topicCommentTemplate.html');
    ...
    $.ajax({
    	...
    	success:function(data){
    		if(data.returnStatus=='000'){//返回成功
    		   ...
    		   $('.headiconintotem').setTemplateURL(projectName+'manage/template/topicLoadMoreTemplate.html',null, {filter_data: false});
...
function cancelnewreply(obj,type){
	...
	$.ajax({
       	...
       	success:function(data){
       		if(data.returnStatus=='000'){
//       			if(type=='comment')
       			$this.parents('.zm-item-answer').remove();

...
$(function(){
   ...
   //查看他人主页  点击个人分享
    ...
    $('a[name=saveusername]').live('click',function(){
    	var username=$('#nameeditsaveusername').val();
    ...
    $('a[name=savecompname]').live('click',function(){
    	var compname=$('#nameeditsavecompname').val();
    	
    	if($.trim(compname).length==0){
    		$('.editcompname').hide();
    		$('.compname').text('填写公司名称');
    	}else{
    ...
    $('a[name=saveposition]').live('click',function(){
    	var position=$('#nameeditsaveposition').val();
    	if($.trim(position).length==0){
    		$('.editposition').hide();
    		$('.position').text('填写职位');
    	}else{
    ...
    $('a[name=saveindustry]').live('click',function(){
    	var industry=$('#nameeditsaveindustry').val();
    ...
    //技能特长获取焦点
	$('.manage-skills').live('click',function(){
		$('#searchSkill').focus();
	});
    //修改城市或者省份
...
function intoUserInfo(){
	$('.uhead,.zm-item-link-avatar,.author-link,.zm-item-img-avatar').pinwheel();
}

...
$(function(){
    ...
    $('#zh-question-title a[name=edit]').live('click',function(){
      ...
      $("#titletemplate").empty();
      $('#zh-question-title h2 .zm-editable-content').hide();
      $('#zh-question-title h2 .pj-editable-editor-wrap').show();
    });
    //话题标签点击修改
...
function upReward(obj){
    ...
    //原悬赏值
    var oldRewValue = $('input[name=rewardValue]').val();
    //新
    var newRewValue = $("#reward-val").val();
    //差值
    var Dvalue = parseInt(newRewValue) - parseInt(oldRewValue);
    var topicId = $('input[name=titleid]').val();
    ...
    if($.trim(newRewValue).length==0||$.trim(Dvalue)=='0'){/*||!re.test(parseInt(newRewValue))*/  /*||!isNaN(newRewValue)*/
        $('.zh-pm-warnmsg').html('请提高打赏财富值!').show();
        return false;
    }else if(parseInt(Dvalue)<0 || oldRewValue==null || newRewValue==null){
        //悬赏值减少或者没变
...
function titledisplayorhide(){
    $('#zh-question-title h2 .zm-editable-content').show();
